...
class CourseTeachingStaffListInterface(Ui_CourseAnnouncementInterface, QWidget):
    ...
    def load_data(self):
        ...
        if not self.key in prev_teaching_staff_list or self.current_time - datetime.strptime(prev_teaching_staff_list[self.key]['time'], "%Y-%m-%d %H:%M:%S") >= timedelta(seconds=1):
            try:
                teaching_staff_list_html = self.client.blackboard_course_teaching_staff(self.key)
                self.content = getTables(teaching_staff_list_html)
                self.update_teaching_staff_list_json(self.key, self.current_time, self.content)
    ...
    def display_data(self):
        ...
        for i in range(len(self.content)):
            ...
            tablewidget.setSizePolicy(QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Expanding)
