    ...
    def __init__(self, client: Client, parent=None):
        ...
        if not 'time' in prev_home_page or self.current_time - datetime.strptime(prev_home_page['time'], "%Y-%m-%d %H:%M:%S") >= timedelta(days=30):
                try:
                    ...
                    self.current_courses, self.history_courses, self.announcement, self.organization, self.task = getPortal(home_page)
                    for course in self.current_courses:
                        course['name'] = simplifyCourseName(course['name'])
                    for course in self.history_courses:
                        course['name'] = simplifyCourseName(course['name'])
                    self.update_portal_json(self.current_time, self.current_courses, self.history_courses, self.announcement, self.organization, self.task)
                except:
        ...
        self.currentListWidget.itemClicked.connect(lambda: self.on_listitem_clicked(self.currentListWidget, self.current_courses))
        ...
        self.historyListWidget.itemClicked.connect(lambda: self.on_listitem_clicked(self.historyListWidget, self.history_courses))
